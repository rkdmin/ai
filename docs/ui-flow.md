...
- **정면 사진 업로드 완료 시 자동으로 펼침** + 노트 문구 accent 색 강조
...
### CTA 비활성화 상태 표시
- 버튼 아래 이유 텍스트 항상 표시:
  - 정면 사진 없음 + 퍼컬 미선택 → "정면 사진과 퍼스널컬러를 선택해주세요"
  - 정면 사진 완료 + 퍼컬 미선택 → "퍼스널컬러를 선택해주세요"
  - 정면 사진 없음 + 퍼컬 선택 완료 → "정면 사진을 업로드해주세요"
- 두 조건 모두 충족 시 버튼 활성화, 힌트 텍스트 제거
...
- **2단계 진행 표시**:
  - Step 1 of 2 — 얼굴 측정 중: "얼굴을 분석하고 있어요" + 하단에 "Step 1: MediaPipe 랜드마크 측정  ·  Step 2: AI 스타일 분석"
  - Step 2 of 2 — AI 스타일 분석 중: "코디 카드를 만들고 있어요" + "Step 1: 얼굴 측정 완료 ✓  ·  Step 2: AI 스타일 분석 중"
- 내부 동작: `analyzeFace(imageBase64, additionalImages)` 호출
  - 측면 사진이 있으면 이미지 구성 컨텍스트를 프롬프트 앞에 삽입
  - Gemini가 정면 기준 분석, 측면은 얼굴형 보조 판단
- 성공 → `result`로 이동
...
- `knowsPersonalColor === false`: 선택 UI 없음, 카드 버튼 바로 활성화 (퍼스널컬러 `null`로 전달)
  - 대신 안내 메시지 표시: "퍼스널컬러 없이도 얼굴형 기반 추천을 받을 수 있어요. 나중에 히스토리에서 다시 시도해 보세요."
...
- **카드 잠금 구조**:
  ...
  - Avoid 카드 → 무료 공개 + **한 줄 이유 미리보기** 표시 (클릭 전 신뢰감 형성)
  - 유료 구독자 → 전체 잠금 해제
- 광고 시청 완료 → 해당 rank 카드 **잠금 해제 피드백**: accent 좌측 테두리 + "✓ 해제됨" 배지 표시
...
> ⚠️ **상태에 따라 둘 중 하나만 표시** (동시에 표시하지 않음)
> - 비구독자: 유료 유도 블록만 → 버튼 클릭 시 구독 유도 모달
> - 구독자 (생성 전): "사진 생성하기" 버튼 활성화
> - 구독자 (생성 후): 전후 비교 토글 UI만 표시

1. "이 스타일을 내 얼굴에 적용해볼까요?" + "사진 생성하기" 버튼 표시
2. 유료 플랜 아닌 경우 → 구독 유도 UI 표시 (모달 아닌 인라인 블록)
...
- **매칭 이유 배지**: 기사 태그 옆에 "✓ 내 퍼스널컬러" / "✓ 내 얼굴형" 배지 표시 → 개인화 신뢰감
...
- 최근 5회 분석 기록 목록
  - 날짜 / 얼굴형 / 퍼스널컬러 (미선택 시 "퍼스널컬러 미선택") / 생성한 카드 타입
