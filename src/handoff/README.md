...
> ⚠️ **이 핸드오프는 이미 적용 완료됐다.** 아래 절차는 당시 기록이며 다시 실행하면
> 그 뒤의 모든 작업(Phase 4~6)이 덮어써진다. **절대 다시 복사하지 마라.**
>
> - 현재 살아있는 코드는 `src/` 다. 이 폴더의 `src/` 는 당시 스냅샷 사본이다.
> - 그래서 `src/handoff/**` 는 lint 대상에서 제외되어 있다 (`eslint.config.js`).
> - 디자인 시스템의 현재 규칙은 `.claude/rules/frontend.md` 를 본다.
> - 여전히 유효한 것은 `CLAUDE_CODE_NOTES.md` 의 **퍼블리시티권 정책 경위**뿐이다.
...
<!-- check-file:allow-policy-terms — 아래는 금지 항목을 나열하는 목록이다 (구현이 아니다). -->
